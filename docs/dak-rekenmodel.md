...
**Status:** veldklaar referentiedocument. Conventies: `B` = overspanning loodrecht op de nok (m), `L` = maat evenwijdig aan de nok (m), `a` = hellingshoek t.o.v. horizontaal (graden), `F` = horizontale footprint van de bovenste (niet-kelder) verdieping (m²), `h` = nokhoogte boven muurplaat/knieschot (m). Alle maten **binnenwerks** (ISSO 82.1 §8.2), oppervlakten op 2 decimalen.
